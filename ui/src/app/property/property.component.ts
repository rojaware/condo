...
export class PropertyComponent extends BaseComponent implements OnInit {
  ...
  createProperty(): void {
    this.propertyService.create(this.currentProperty).subscribe({
      next: (res) => {
        console.log(res);
   ...
   */
  onStartDateClose() {
    const endDate = this.util.calculateNextFiscalYearEndDate(
  ...
  onExtendedDateFocused(event: any) {
    if (this.util.isEmpty(this.currentProperty.extendedEndDate)) {
      if (!this.util.isEmpty(this.currentProperty.endDate)) {
        const extendedEndDate = this.util.calculateNextFiscalYearEndDate(
